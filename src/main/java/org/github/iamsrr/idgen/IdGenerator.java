...
public class IdGenerator {
    ...
    static {
        ...
        InputStream in = null;
        try {
            in = IdGenerator.class.getClassLoader().getResourceAsStream("application.yml");
            prop.load(in);
            datacenterId = Integer.parseInt(prop.getProperty("IdGen.datacenter"));
            workerId = Integer.parseInt(prop.getProperty("IdGen.worder"));
        } catch (Exception e) {
            // datacenterId 默认为0
            datacenterId = 0;
            // workerId 默认为1
            workerId = 1;
            //LOG.error("ID生成器配置读取失败，使用默认值: datacenterId=0，workerId=1，建议检查后重新启动应用");
        } finally {
            if (in != null) {
                try {
                    in.close();
                } catch (IOException e) {
                    //LOG.error("id生成器配置文件读取流关闭失败{%s}", e.getMessage()));
                    e.printStackTrace();
                }
            }
        }
